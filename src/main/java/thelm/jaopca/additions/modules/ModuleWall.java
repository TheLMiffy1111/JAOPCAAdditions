...
public class ModuleWall extends ModuleBase {
	...
	public void init() {
		for(IOreEntry entry : JAOPCAApi.ENTRY_NAME_TO_ORES_MAP.get("wall")) {
			GameRegistry.addRecipe(new ShapedOreRecipe(Utils.getOreStack("wall", entry, 12), new Object[] {
					"BBB",
					"BBB",
	...
	public static class BlockWallBase extends BlockBase {
		...
		public BlockWallBase(Material material, MapColor mapColor, ItemEntry itemEntry, IOreEntry oreEntry) {
		...
		public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos) {
			...
			return block == Blocks.BARRIER ? false : (!(block instanceof BlockWallBase) && !(block instanceof BlockFenceGate) ? (iblockstate.getMaterial().isOpaque() && iblockstate.isFullCube() ? iblockstate.getMaterial() != Material.GOURD : false) : true);
		}
